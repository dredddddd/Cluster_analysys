...
double Field::fun(double x, double y) 
{ 
    return (x * x - y * y) / 10 + 5; 
//    return (x * y) / 2 + 3 * x + 10;
}
...
void Field::function_interpolation(Vector p)
{
    ...
    else
    {
        ...
        for (i = 0; i < neighbouring_points.size(); i++)
        {
            ...
            sum_w = sum_w + w[i];
            cout << neighbouring_points[i].get_first_coordinate() << " " << neighbouring_points[i].get_second_coordinate() << " " << w[i] <<
                " " << neighbouring_points[i].get_function_value() << endl;
        }
        cluster.pop_back();
